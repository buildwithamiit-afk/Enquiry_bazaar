...
export function AboutHero() {
  return (
    <section className={`relative bg-gradient-to-b from-[#0B1E3D] via-[#0D2447] to-[#0B1E3D] text-white py-14 sm:py-20 ${inter.className}`}>
      
      {/* Ambient Lighting */}
      <div className="pointer-events-none absolute inset-0 -z-0 overflow-hidden select-none">
        <div className="absolute -top-24 left-1/2 -translate-x-1/2 h-[400px] w-[800px] rounded-full bg-blue-500/10 blur-[80px]"></div>
        <div className="absolute top-1/2 -right-20 h-72 w-72 rounded-full bg-cyan-400/10 blur-[80px]"></div>
      </div>
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="flex justify-center">
          <div className="inline-flex items-center gap-2 rounded-full border border-blue-400/30 bg-blue-500/10 px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-blue-300 backdrop-blur-md shadow-2xs">
        ...
        <div className="mt-4 text-center max-w-3xl mx-auto">
          <h1 className={`text-2xl font-extrabold tracking-tight text-white sm:text-4xl lg:text-[2.65rem] leading-[1.2] ${montserrat.className}`}>
            Helping Indian Manufacturers Get <span className="text-[#1E5EFF]">Direct Bulk Orders</span> Without Middlemen.
          </h1>
        ...
        <div className="mt-12 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="rounded-2xl border border-white/10 bg-white/5 p-4 sm:p-5 text-center backdrop-blur-xs transition hover:border-blue-400/40 hover:bg-white/[0.08]"
            >
              ...
              <div className="mt-1 text-xs sm:text-[13px] font-bold text-blue-200">
                {stat.label}
              </div>
